...
| `diagnostics.py` | `run_diagnostics` — the 4-combo (hedge × logic) decomposition table | config, data_loader, factors, backtest |
...
## Decomposition diagnostic (run this before trusting any number)

`diagnostics.py` runs the backtest four times — every combination of
`enable_hedge` × `enable_logic_fusion` — on one in-memory panel and prints a
comparison table (CAGR / Sharpe / MaxDD / periods traded / flat / hit-rate):

- terminal: `python diagnostics.py`
- Jupyter, reusing a panel you already built:
  ```python
  from diagnostics import run_diagnostics
  res = run_diagnostics(panel, feats)        # skips reloading; ~seconds per combo
  ```

It restores your original `CONFIG` toggles afterward. Read the first row
(hedge OFF, logic OFF) as the raw model alpha; a large `Flat` count means many
periods were blocked and booked as 0.0, which inflates Sharpe — weigh the two together.
